...
unsigned int hook_nat_in(void *priv, struct sk_buff *skb,
  ...
  if (conn == NULL) { // 不应出现连接表中不存在的情况
    printk(
        KERN_WARNING
        "[fw nat] (in)get a connection that is not in the connection pool!\n");
    return NF_ACCEPT;
  }
  // 无记录->返回
  ...
  case IPPROTO_TCP:
    ...
    tcpHeader->check =
        csum_tcpudp_magic(header->saddr, header->daddr, tot_len - hdr_len,
...
unsigned int hook_nat_out(void *priv, struct sk_buff *skb,
                          const struct nf_hook_state *state) {
  ...
  if (conn == NULL) { // 不应出现连接表中不存在的情况
    printk(
        KERN_WARNING
        "[fw nat] (out)get a connection that is not in the connection pool!\n");
    return NF_ACCEPT;
  }
  // 确定NAT记录
