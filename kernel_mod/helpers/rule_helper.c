...
void *formAllIPRules(unsigned int *len) {
  ...
  *len = sizeof(struct KernelResponseHeader) + sizeof(struct IPRule) * count;
  ...
  head->arrayLen = count;
  for (now = ipRuleHead, p = (mem + sizeof(struct KernelResponseHeader));
       now != NULL; now = now->nx, p = p + sizeof(struct IPRule))
...
struct IPRule matchIPRules(struct sk_buff *skb, int *isMatch) {
  ...
  *isMatch = 0;
  ...
  read_lock(&ipRuleLock);
  for (now = ipRuleHead; now != NULL; now = now->nx) {
    if (matchOneRule(now, ntohl(header->saddr), ntohl(header->daddr), sport,
                     dport, header->protocol)) {
      ret = *now;
      *isMatch = 1;
      break;
    }
