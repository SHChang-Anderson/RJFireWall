...
// 检查是否存在指定连接
struct connNode *hasConn(unsigned int sip, unsigned int dip,
...
// 新建连接
struct connNode *addConn(unsigned int sip, unsigned int dip,
                         unsigned short sport, unsigned short dport,
                         u_int8_t proto, u_int8_t log) {
  // 初始化
  struct connNode *node =
      (struct connNode *)kzalloc(sizeof(connNode), GFP_ATOMIC);
  if (node == NULL) {
...
void *formAllConns(unsigned int *len) {
  ...
  read_lock(&connLock);
  // 计算总量
  for (node = rb_first(&connRoot), count = 0; node;
       node = rb_next(node), count++)
    ;
  // 申请回包空间
  *len = sizeof(struct KernelResponseHeader) + sizeof(struct ConnLog) * count;
  ...
  for (node = rb_first(&connRoot); node;
       node = rb_next(node), p = p + sizeof(struct ConnLog)) {
    now = rb_entry(node, struct connNode, node);
    log.saddr = now->key[0];
    log.daddr = now->key[1];
...
int eraseConnRelated(struct IPRule rule) {
  ...
  while (hasChange) { // 有更改时，持续遍历，防止漏下节点
    ...
    for (node = rb_first(&connRoot); node; node = rb_next(node)) {
      ...
      if (matchOneRule(&rule, needDel->key[0], needDel->key[1], sport, dport,
                       needDel->protocol)) { // 相关规则
        hasChange = 1;
        break;
      }
    }
...
#endif
  conn_timer.expires =
      timeFromNow(CONN_ROLL_INTERVAL); // 超时时间设置为CONN_ROLL_INTERVAL秒后
  add_timer(&conn_timer);              // 激活定时器
}

// 关闭连接池
void conn_exit(void) { del_timer(&conn_timer); }
