...
struct KernelResponse addFilterRule(char *after, char *name, char *sip,
                                    char *dip, unsigned int sport,
                                    unsigned int dport, u_int8_t proto,
                                    unsigned int log,
                                    unsigned int action); // 新增一条过滤规则，其中，sport/dport为端口范围：高2字节为最小
                                                          // 低2字节为最大
struct KernelResponse delFilterRule(char *name);
struct KernelResponse getAllFilterRules(void);
struct KernelResponse addNATRule(char *sip, char *natIP, unsigned short minport,
