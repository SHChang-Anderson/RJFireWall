...
struct KernelResponse exchangeMsgK(void *smsg, unsigned int slen) {
  ...
  struct nlmsghdr *message =
      (struct nlmsghdr *)malloc(NLMSG_SPACE(slen) * sizeof(uint8_t));
  if (!message) {
  ...
  // send msg
  if (!sendto(skfd, message, message->nlmsg_len, 0, (struct sockaddr *)&kpeer,
              sizeof(kpeer))) {
  ...
  // recv msg
  struct nlmsghdr *nlh =
      (struct nlmsghdr *)malloc(NLMSG_SPACE(MAX_PAYLOAD) * sizeof(uint8_t));
  if (!nlh) {
